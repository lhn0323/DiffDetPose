...
class DynamicHead(nn.Module):
    ...
    def __init__(self, cfg, roi_input_shape):
        ...
        rcnn_head = RCNNHead(cfg, d_model, num_proposals, num_classes, dim_feedforward, nhead, dropout, activation)
    ...
    def forward(self, features, init_bboxes, t, init_features):
        ...
        for head_idx, rcnn_head in enumerate(self.head_series):
            class_logits, pred_bboxes, proposal_features, pred_pose = rcnn_head(features, bboxes, proposal_features, self.box_pooler, time) 
...
class RCNNHead(nn.Module): 
    ...
    def __init__(self, cfg, d_model,num_proposals, num_classes, dim_feedforward=2048, nhead=8, dropout=0.1, activation="relu",
        ...
        self.linear_pose1 = nn.Linear(d_model, dim_feedforward) 
        ...
        # pose_T
        num_pose_T = cfg.MODEL.DiffusionDet.POSE_T 
        pose_module_T = list()
        ...
        self.pose_delta_R = nn.Linear(d_model,3) 
        self.pose_delta_T = nn.Linear(d_model,3) 
    ...
    def forward(self, features, bboxes, pro_features, pooler, time_emb):
        ...
        pose_feature_R = pose_feature.clone() 
        pose_feature_T = pose_feature.clone()
    ...
    def pose(self,N, nr_boxes, pro_features, roi_features, time_emb):
        ...
        # self_att.Self-Attention
        pro_features = pro_features.view(N, nr_boxes, self.d_model).permute(1, 0, 2)
        pro_features2 = self.self_pose_attn(pro_features, pro_features, value=pro_features)[0] 
        pro_features = pro_features + self.dropout1(pro_features2)
        pro_features = self.norm_pose1(pro_features)

        # inst_interact.
        pro_features = pro_features.view(nr_boxes, N, self.d_model).permute(1, 0, 2).reshape(1, N * nr_boxes, self.d_model) 
        pro_features2 = self.inst_pose_interact(pro_features, roi_features)
        pro_features = pro_features + self.dropout2(pro_features2)
        obj_features = self.norm_pose2(pro_features)

        # obj_feature.        
        obj_features2 = self.linear_pose2(self.dropout(self.activation(self.linear_pose1(obj_features)))) 
        obj_features = obj_features + self.dropout3(obj_features2)
        obj_features = self.norm_pose3(obj_features)
        
        fc_feature = obj_features.transpose(0, 1).reshape(N * nr_boxes, -1)

        scale_shift = self.block_time_mlp_pose(time_emb)
        scale_shift = torch.repeat_interleave(scale_shift, nr_boxes, dim=0)
        scale, shift = scale_shift.chunk(2, dim=1)
        fc_feature = fc_feature * (scale + 1) + shift
